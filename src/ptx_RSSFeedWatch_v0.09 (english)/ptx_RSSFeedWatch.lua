...
local ucmd_menu_mutefeed = { "RSSFeedWatch", "Mute a feed" }
...
local Order = {"feeds","listusers","listfeeds","lastfeed","addfeed","forcefeed","delfeed","feedhelp","getfeed","mutefeed","refresh","maxcache","maxcache","settings"}
...
FwCmds = {
		...
		if user then
			if next(Feeds) then
				...
				for i,v in ipairs(Feeds) do
					...
					reply = reply.."\t"..utf_format("[%-2s]    ",i)..v.url..ismuted.."\n"
				end
				if reply ~= "" then
					local PM = private
					if not PM then
	...
	mutefeed = {function(user,data,cmd2,data2,private)
		if user then
			...
			return "Mute a Feed <Feed number>","",""
		end
	end,
	{ level = 10 } --min level to use this command
	},
	...
	settings = {function(user,data,cmd2,data2,private)
		if user then
			local reply,c = "\n\n\t"..scriptname.." Settings\n\n\t"..utf_format("%-22s","Setting")..
			"\tValue\r\n\t"..string_rep("-",50).."\r\n",""
			for k,v in pairs(tSettings) do
				c = c.."\t"..utf_format("%-22s",k).."\t"..tostring(v).."\r\n"
			end
			if utf_len(c) > 0 then
				local PM = private
				if not PM then
