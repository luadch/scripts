...
local msg_help = lang.msg_help or [[
...
hub.setlistener( "onStart", { },
    function( p )
        ...
        if ucmd then
            ...
            ucmd.add( { ucmd_menu,"Movies","4k",ucmd_add }, cmd_Add, { "Movies_4k", "%[line:Name:]" }, { "CT1" }, Add )
            ucmd.add( { ucmd_menu,"Movies","4k",ucmd_show }, cmd_Show, { "Movies_4k"}, { "CT1" }, Show )
            ...
            ucmd.add( { ucmd_menu,"TV","4k",ucmd_add }, cmd_Add, { "TV_4k", "%[line:Name:]" }, { "CT1" }, Add )
            ucmd.add( { ucmd_menu,"TV","4k",ucmd_show }, cmd_Show, { "TV_4k"}, { "CT1" }, Show )
            ...
            ucmd.add( { ucmd_menu,"Dokus","4k",ucmd_add }, cmd_Add, { "Dokus_4k", "%[line:Name:]" }, { "CT1" }, Add )
            ucmd.add( { ucmd_menu,"Dokus","4k",ucmd_show }, cmd_Show, { "Dokus_4k"}, { "CT1" }, Show )
            ...
            ucmd.add( { ucmd_menu,"XXX","4k",ucmd_add }, cmd_Add, { "XXX_4k", "%[line:Name:]" }, { "CT1" }, Add )
            ucmd.add( { ucmd_menu,"XXX","4k",ucmd_show }, cmd_Show, { "XXX_4k"}, { "CT1" }, Show )
            ...
            ucmd.add( { ucmd_menu,"Menu",ucmd_search_release }, cmd_Search,{ "%[line:]" }, { "CT1" }, Search )
        ...
        if os.difftime( os.time( ) - start ) >= 60 then
            if FreshStuff.WhenAndWhatToShow[ os.date( "%H:%M" ) ] then
                if FreshStuff.Types[ FreshStuff.WhenAndWhatToShow[ os.date( "%H:%M" ) ] ] then
                    hub_broadcast( FreshStuff.ShowRelType( FreshStuff.WhenAndWhatToShow[ os.date( "%H:%M" ) ] ), botname() )
                else
                    if FreshStuff.WhenAndWhatToShow[ os.date( "%H:%M" ) ] == "new" then
                        hub_broadcast( FreshStuff.MsgNew, botname() )
                    elseif FreshStuff.WhenAndWhatToShow[ os.date( "%H:%M" ) ] == "all" then
                        hub_broadcast( FreshStuff.MsgAll, botname() )
                    else
...
function FreshStuff.AddCrap( user, data, env )
    ...
    if cat then
        if FreshStuff.Types[ cat ] then
            ...
            hub_broadcast( user:nick() .. msg_add_crap_04 .. cat .. msg_add_crap_05 .. tune, botname() )
...
function FreshStuff.AddCrapAnnounce( user, data, env )
    ...
    if cat then
        if FreshStuff.Types[ cat ] then
            ...
            SendTxt( user, env, botname(), tune .. msg_add_crap_03 .. cat )
            hub_broadcast( alibi .. msg_add_crap_04 .. cat .. msg_add_crap_05 .. tune, botname() )
            FreshStuff.Count = FreshStuff.Count + 1
            FreshStuff.AllStuff[ FreshStuff.Count ] = { cat, alibi, os.date( "%m/%d/%Y" ), tune }
            FreshStuff.SaveRel()
            FreshStuff.ReloadRel()
            refresh_bot()
        else
