...
-- Always send these feeds to all
local ForceFeed = {
	[1] = false,
	[2] = false,
	[3] = false,
	...
	for item in string.gmatch(xml, "<item>(.-)</item>") do
		...
		for field,val in string.gmatch(item, "(%b<>)(.-)%<%/") do
			...
			if Fields[field:lower()] then
				...
				if string_len(val) > MaxWidth then 
					s = s..string_sub(val,1,MaxWidth).."..." 
				else 
					s = s..val.."" 
				end
			end
			if string_len(s) > 0 then table.insert(t,s) end
		end
...
GetFeed = function(n)
	...
	if fd and sz then
		...
		if msg_ and string_len(msg_) > 0 then
			if not Simple then msg_ = msg_.."\t\t\tProcessed In: "..time.."\n\n" end
			if ForceFeed[n] then
				if ForceFeedPM then
					hub_broadcast(msg_, Bot, Bot)
				else
...
FwCmds = {
	...
	listusers = {function(user,data,cmd2)
		if user then
			...
			c = table.concat(t,"")
			if string_len(c) > 0 then return reply..c.."\n\t"..r:rep(50).."\r\n\r\n" end
		else
