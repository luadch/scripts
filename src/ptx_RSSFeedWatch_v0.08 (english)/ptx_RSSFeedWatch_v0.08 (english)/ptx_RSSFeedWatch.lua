...
hub.setlistener( "onTimer", {},
	function()
		if StartTime and (os_time() - StartTime) >= Refresh*60 then
			StartTime = os_time()
			if #Feeds > 0 then
...
hub.setlistener( "onStart", { },
	function( )
		StartTime = os_time()
		MaxFeeds,Refresh = math.min(MaxFeeds,MaxCache),math.max(Refresh,2)
		if StartFeed > #Feeds then StartFeed = 1 end
		...
		if GetFeedAtStart and #Feeds > 0 then
			if allFeedsAtOnce then
				for n,v in ipairs(Feeds) do
					GetFeed(n)
				end
