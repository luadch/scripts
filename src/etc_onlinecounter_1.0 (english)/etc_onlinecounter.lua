...
local tSettings = {
	...
	-- Maximum hubbers to show when using !onlinecheck
	iMax = 30,
...
local tCommands = {
	toponline = {
		fFunction = function(user, data)
			...
			if next(tOnlineCounter) then
				...
				user:reply("*** Error: Check Online's table is currently empty!", tSettings.sBot)
			end
			return PROCESSED
		end,
		minLevel = 10,
	...
	userlowuptime = {
		...
		minLevel = 60,
		tRC = { { { "Show Users With To Low Online Time" }, { }, { "CT1" } } }
	},
}
