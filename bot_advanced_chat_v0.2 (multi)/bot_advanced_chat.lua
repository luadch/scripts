...
local help_usage = lang.help_usage or "[+!#]advancedchat add|del <nick>  /  [+!#]advancedchat members  / in the chat: [+!#]members|[+!#]history|[+!#]historyall"
...
local ucmd_menu_ct1_historyall = lang.ucmd_menu_ct1_historyall or { "User", "Messages", "Chats", "[CHAT]AdvancedChat", "show chat history (all)" }
...
buildlog = function( amount_lines, login )
...
onbmsg = function( user, command, parameters )
    ...
    if param2 == cmd_p_history then
        user:reply( buildlog( default_lines, false ), hub_getbot() )
        return PROCESSED
    end
    if param2 == cmd_p_historyall then
        user:reply( buildlog( max_lines, false ), hub_getbot() )
        return PROCESSED
    end
            ...
            if targetuser == advancedchat then
                ...
                if checkPermission( user ) then
                    ...
                    if cmd == cmd_history then
                        user:reply( buildlog( default_lines, false ), advancedchat, advancedchat )
                        return PROCESSED
                    end
                    if cmd == cmd_historyall then
                        user:reply( buildlog( max_lines, false ), advancedchat, advancedchat )
                        return PROCESSED
                    end
                end
            end
