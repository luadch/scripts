...
local help_usage = lang.help_usage or "[+!#]blackboard [addinfo <userSID> <info> |showusers all|showinfo <userSID>|"
local help_desc = lang.help_desc or "Allows you to [ add info about user| shows all users with infos added | shows all infos of a user."
...
local onbmsg = function( user, adccmd, parameters)
	...
	if user_level < min_level then 
		...
		if id == prm1 then	-- addinfo
			user:reply( adduserinfo( target, data ), hub_bot )
			return PROCESSED
		end
...
adduserinfo = function ( target, item)
	local msg
